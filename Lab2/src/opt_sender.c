...
int sender(char *receiver_ip, char* receiver_port, int window_size, char* message){
  ...
  memset(ACK, 0, window_size * sizeof(bool));
  ...
  if((fd = open(message, O_RDWR)) != -1){
    ...
    while(true) {
      ...
      if(rtp->type == RTP_ACK){
        ...
        if(rtp->seq_num >= start && rtp->seq_num < end)
          ACK[rtp->seq_num % window_size] = true;

        if(start == rtp->seq_num) {
          while(ACK[start % window_size]){
            ACK[start % window_size] = false;
            start++;
          }
          send_time = clock();
        }
